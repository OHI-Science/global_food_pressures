...
```{r}
rescaling_data <- read_csv(here("_analysis/scenarios/fisheries_disturbance_rescale_values.csv"))
...
stressor_system_files <- list.files("/home/shares/food-systems/Food_footprint/all_food_systems/scenarios/scenarios_analysis/scenarios_tif_equal_area_proj_per_cell", pattern=stressor, full=TRUE)
...
rescaled_raster = raster::calc(stressor_raster, fun=function(x){ifelse(x>0, 
                                      (x/resc_num) * 1000000, 
                                       0)})
writeRaster(rescaled_raster, sprintf("/home/shares/food-systems/Food_footprint/all_food_systems/scenarios/scenarios_analysis/scenarios_tif_equal_area_proj_per_cell_rescaled/%s", file_name),
              overwrite=TRUE)
}
}
