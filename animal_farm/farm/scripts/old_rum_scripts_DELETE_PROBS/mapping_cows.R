## Function creating maps of cows distribution: grassland and mixed; milk and meat
## Author: Juliette Verstaen


mapping_cows <- function(animal){
  
  for(system in cows_system_list) {
    for(product in cows_product_list) {
      ...
      ## grab the system proportions calculated from the gridded livestock maps
      ...
      livestock_map_cell_prop <- livestock_map_system %>% 
        group_by(iso3c) %>% 
        mutate(rgn_total = sum(cell_count_all, na.rm = TRUE)) %>% 
        ungroup() %>% 
        rowwise() %>% 
        mutate(map_prop = cell_count_syst_gf/rgn_total) %>% 
        ungroup()
      ...
      final_map <- livestock_map_cell_prop %>% 
        ...
        mutate(current_count = map_prop*fao_headcount,
               product = product_type) %>% 
        select(x,y, iso3c, country, animal, system, product, cell_headcount_all = cell_count_all, cell_headcount_system = cell_count_syst_gf, rgn_total, map_prop, fao_headcount, current_count)
      
      
      write_csv(final_map, file.path(prep, paste0("animal_farm/farm/ungapfilled/", animal_type, "_",  system_type, "_", product_type, "_location_df_ungf.csv"))) 
